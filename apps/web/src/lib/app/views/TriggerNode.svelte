<script lang="ts">
  ...
  const isSourceKind = $derived(kind === 'envelope' || kind === 'lfo' || kind === 'cc' || kind === 'note' || kind === 'osc' || kind === 'audio' || kind === 'randomMod');
...
{#snippet sourceThumb()}
  ...
  {:else if node && node.kind === 'audio'}
    <NodeSignalPreview kind="audio" ccValue={() => store.audioNodeLiveValue(node)} w={56} h={32} />
